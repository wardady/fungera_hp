...
void Memory::set_region_allocation(const std::array<size_t, 2> &ip,
    ...
    for (size_t i = ip[0]; i < ip[0] + size[0]; ++i)
        for (size_t j = ip[1]; j < ip[1] + size[1]; ++j) {
            (*this)(i, j).free = value;
            emit memory_cell_changed(i, j, (*this)(i, j).instruction, value); //! Deadlock here if BlockingQueuedConnection is used
        }
}
